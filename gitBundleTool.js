...
const DEFAULT_SERVER_URL = process.env.GIT_BUNDLE_SERVER || 'http://14.225.222.181:5000/api/bundles';
const DEFAULT_REMOTE = process.env.GIT_REMOTE || 'origin';
const DEFAULT_TOKEN = process.env.GIT_BUNDLE_TOKEN || process.env.BUNDLE_TOKEN;
...
function uploadBundle({ bundlePath, serverUrl, remoteUrl, remoteName, branch, headSha, author, repo, token }) {
...
async function cleanupTempDir(dirPath, attempt = 0) {
  try {
    await fsp.rm(dirPath, { recursive: true, force: true });
  } catch (error) {
    if (error.code === 'ENOTEMPTY' && attempt < 5) {
      const backoffMs = 100 * (attempt + 1);
      await new Promise((resolve) => setTimeout(resolve, backoffMs));
      return cleanupTempDir(dirPath, attempt + 1);
    }
    throw error;
  }
}
...
async function main() {
  ...
  if (!token) {
    throw new Error('Missing bundle server token. Provide --token or set GIT_BUNDLE_TOKEN.');
  }
  ...
  try {
    ...
    await cleanupTempDir(tempDir).catch((error) => {
      console.warn(`Failed to remove temp dir ${tempDir}:`, error.message);
    });
  }
}
