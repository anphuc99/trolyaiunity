...
function parseArgs(argv) {
  const args = {};const fs = require('fs');
const path = require('path');
const os = require('os');
const { execFileSync } = require('child_process');
const http = require('http');
...
async function main() {
  ...
  const token = fs.existsSync('servertoken.txt') ? fs.readFileSync('servertoken.txt', 'utf8').trim() : (args.token || process.env.GIT_BUNDLE_TOKEN || process.env.BUNDLE_TOKEN);
