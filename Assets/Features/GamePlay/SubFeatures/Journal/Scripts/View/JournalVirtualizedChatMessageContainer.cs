...
namespace Features.GamePlay.SubFeatures.Journal.View
{
	...
	/// </summary>
	public sealed class JournalVirtualizedChatMessageContainer : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollHandler
	{
		public Action<JournalMessageBubbleData> OnMessageSpeakerClicked { get; set; }
		public Action<JournalMessageBubbleData> OnMessageSpeakerLongPressed { get; set; }
		public Action<JournalMessageBubbleData> OnMessageTranslateClicked { get; set; }
		...
		[SerializeField]
		private JournalMessageBubble _characterMessagePrefab;

		[SerializeField]
		private JournalMessageBubble _userMessagePrefab;
		...
		[SerializeField]
		private List<JournalMessageBubbleData> _messages = new();

		private readonly List<JournalMessageBubble> _characterPool = new();
		private readonly List<JournalMessageBubble> _userPool = new();
		private readonly List<float> _messageHeights = new();
		private readonly List<float> _prefixHeights = new();

		private JournalMessageBubble _characterMeasureBubble;
		private JournalMessageBubble _userMeasureBubble;
		...
		public void SetMessages(IReadOnlyList<JournalMessageBubbleData> messages)
		{
			_messages.Clear();
			if (messages != null)
			{
				...
				{
					_messages.Add(messages[i] ?? new JournalMessageBubbleData());
				}
			}
		...
		public void AddNewMessage(JournalMessageBubbleData message)
		{
			_messages.Add(message ?? new JournalMessageBubbleData());
			SyncMessageIndices();
			RebuildMetrics();
		...
		public void UpdateCharacterAvatar(string characterName, Sprite avatar)
		{
			...
			for (var i = 0; i < _messages.Count; i++)
			{
				...
				if (message == null || message.Type != JournalMessageBubbleType.Character)
				{
					continue;
				}
		...
		public void ToggleMessageTranslation(JournalMessageBubbleData messageData)
		{
			if (messageData == null)
			{
				return;
		...
		private void ToggleMessageTranslationInternal(JournalMessageBubbleData targetMessage, int messageIndex, string translation)
		{
		...
		/// </summary>
		/// <param name="message">Message text.</param>
		public void AddOldMessage(JournalMessageBubbleData message)
		{
			_messages.Insert(0, message ?? new JournalMessageBubbleData());
			SyncMessageIndices();
			RebuildMetrics();
		...
		private void RefreshVisible()
		{
			...
			for (var slot = 0; slot < _characterPool.Count; slot++)
			{
				...
				var targetBubble = data.Type == JournalMessageBubbleType.User ? userBubble : characterBubble;
				var otherBubble = data.Type == JournalMessageBubbleType.User ? characterBubble : userBubble;
		...
		private JournalMessageBubble CreatePooledBubble(JournalMessageBubble prefab, string suffix)
		{
		...
		private void PrepareMeasureBubble(JournalMessageBubble bubble, string suffix)
		{
			if (bubble == null)
			{
				return;
		...
		private void HandleBubbleSpeakerClicked(JournalMessageBubbleData messageData)
		{
			OnMessageSpeakerClicked?.Invoke(messageData);
		}

		private void HandleBubbleSpeakerLongPressed(JournalMessageBubbleData messageData)
		{
			OnMessageSpeakerLongPressed?.Invoke(messageData);
		}

		private void HandleBubbleTranslateClicked(JournalMessageBubbleData messageData)
		{
		...
		private float MeasureMessageHeight(JournalMessageBubbleData data)
		{
			...
			var measureBubble = data.Type == JournalMessageBubbleType.User ? _userMeasureBubble : _characterMeasureBubble;
